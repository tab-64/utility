...
public:
    ...
    MessagePtr back(void){
        if(!msg_queue_.empty()){
            return std::move(msg_queue_.back());
        }
        else{
...
public:
    ...
    MessagePtr get_front(void){
        ...
        return std::move(MessageQueue::get_front());
    }
    ...
    MessagePtr&& back(void){
        std::lock_guard<std::mutex> lock(mtx_);
        return std::move(MessageQueue::back());
    }
    ...
    MessageQueue_threadsafe& operator<<(const MessagePtr& rhs){
        push(rhs);  // Use this thread-safe method.
        return *this;
    }
    MessageQueue_threadsafe& operator>>(MessagePtr& rhs){
        std::lock_guard<std::mutex> lock(mtx_);
